...
func (a *ApiController) ChainAdd(ctx *gin.Context) {
	...
	if isSuccess {
		l := strings.Split(chain.PeersOrgs, ",")
		for i := 0; i < len(l); i++ {
			org := entity.Org{Name: l[i], Chain: chain.Id, Publickey: "defaultkey"}
			a.orgService.Add(&org)
		}
...
func (a *ApiController) ChainDeleted(ctx *gin.Context) {
	...
	if isSuccess {
		l := strings.Split(chain.PeersOrgs, ",")
		for i := 0; i < len(l); i++ {
			org := entity.Org{Name: l[i], Chain: chain.Id}
			a.orgService.Delete(&org)
		}
		gintool.ResultMsg(ctx, msg)
	} else {
		gintool.ResultFail(ctx, msg)
